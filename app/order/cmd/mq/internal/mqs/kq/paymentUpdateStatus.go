...
import (
	...
	"encoding/json"
	"fmt"
	"looklook_study/app/order/cmd/mq/internal/svc"
	"looklook_study/app/order/cmd/rpc/order"
	"looklook_study/app/order/model"
	paymentModel "looklook_study/app/payment/model"
...
func (l *PaymentUpdateStatusMq) execService(message kqueue.ThirdPaymentUpdatePayStatusNotifyMessage) error {
	...
	if orderTradeState != -99 {
		...
		if err != nil {
			return errors.Wrapf(xcode.NewErrMsg("update homestay order state fail"), "update homestay order state fail err : %v ,message:%+v", err, message)
		}
	}
