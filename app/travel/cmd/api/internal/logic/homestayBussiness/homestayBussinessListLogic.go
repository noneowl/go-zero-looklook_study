...
func (l *HomestayBussinessListLogic) HomestayBussinessList(req *types.HomestayBussinessListReq) (*types.HomestayBussinessListResp, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(xcode.NewErrCode(xcode.DB_ERROR), "HomestayBussinessList FindPageListByIdDESC db fail ,  req : %+v , err:%v", req, err)
	}

	var resp []types.HomestayBusinessListInfo
	if len(list) > 0 {
		for _, homestayBussiness := range list {
			var typeHomestayBusinessListInfo types.HomestayBusinessListInfo
			_ = copier.Copy(&typeHomestayBusinessListInfo, homestayBussiness)
